...
date: XVIII Jornada de Investigación Científica, UASD <br> 12, 13 y 14 de noviembre, 2019 <br> Santo Domingo, República Dominicana <br> https://geofis.github.io/granulometria-aluvial-superficial-mediante-uav/
output:
...
* La carga gruesa son materiales de dimensiones __mayores a las del grano de arena__, transportados por cursos fluviales __mediante reptación, arrastre o saltación__.
...
## Medir-detectar bloques

```{r, echo=F, eval=F, warning=F, message=F}
library(RColorBrewer)
hexgrid <- st_make_grid(auto2, square = F, n = c(20,20))
hexgrid <- st_as_sf(hexgrid, gridid=1:length(hexgrid))
hexgridauto <- st_intersection(hexgrid, st_centroid(auto2)) %>%
  group_by(gridid) %>%
  summarise(media=mean(a_auto), max=max(a_auto)) %>% 
  st_drop_geometry()
hexgrid %>% inner_join(hexgridauto) %>% st_write('data/hexgrid.gpkg', driver = 'GPKG')
```

![](img/detectar_bloques.gif)

## Medir-detectar gravilla

## ![](img/detectar_gravilla.gif)
